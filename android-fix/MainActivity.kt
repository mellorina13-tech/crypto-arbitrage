...
class MainActivity : AppCompatActivity() {
    ...
    private fun scanMarkets() {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                // ✅ FIX: Already in coroutine scope, can call suspend function directly
                findRealArbitrageOpportunities()
    ...
    private suspend fun findRealArbitrageOpportunities() = withContext(Dispatchers.IO) {
        ...
        val minVolume = withContext(Dispatchers.Main) {
            when (volumeFilterSpinner.selectedItemPosition) {
                0 -> 0.0
                1 -> 1000000.0
                2 -> 10000000.0
                else -> 100000000.0
